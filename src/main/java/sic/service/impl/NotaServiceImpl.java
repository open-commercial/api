...
public class NotaServiceImpl implements INotaService {
      ...
      if (criteria.getFechaDesde() != null && criteria.getFechaHasta() != null) {
        DateExpression<LocalDateTime> fDesde =
            Expressions.dateTemplate(
                LocalDateTime.class,
                dateTemplate,
                criteria.getFechaDesde().format(formato));
        DateExpression<LocalDateTime> fHasta =
            Expressions.dateTemplate(
                LocalDateTime.class,
                dateTemplate,
                criteria.getFechaHasta().format(formato));
  ...
  @Override
  public TipoDeComprobante[] getTiposNotaCredito(Empresa empresa) {
  ...
  @Override
  public TipoDeComprobante[] getTiposNotaDebito(Empresa empresa) {
    // cuando la Empresa discrimina IVA
    if (CategoriaIVA.discriminaIVA(empresa.getCategoriaIVA())) {
      TipoDeComprobante[] tiposPermitidos = new TipoDeComprobante[5];
  ...
  public RenglonNotaDebito calcularRenglonDebitoConRecibo(Recibo recibo) {
    ...
    String descripcion =
        "Recibo Nº " + recibo.getNumRecibo() + " " + recibo.getFecha().format(formato);
