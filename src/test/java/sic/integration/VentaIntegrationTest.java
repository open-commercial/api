...
public class VentaIntegrationTest {
    ...
    public void setup() {
        ...
        ClienteDTO cliente = ClienteDTO.builder()
                ...
                .telefono("379123452")
                .build();
        restTemplate.postForObject(apiPrefix + "/clientes?idEmpresa=" + empresa.getId_Empresa()
  ...
  private void shouldCrearFacturaB() {
    ProductoDTO productoUno =
  ...
  private void shouldCrearRecibo(double monto) {
  ...
  private void shouldCrearNotaDebito() {
    UsuarioDTO credencial = restTemplate.getForObject(apiPrefix + "/usuarios/1", UsuarioDTO.class);
  ...
  public void testCuentaCorrienteCliente() {
    ...
    List<FacturaVenta> facturasRecuperadas =
        restTemplate
            .exchange(
                apiPrefix
                    + "/facturas/venta/busqueda/criteria?idEmpresa=1&tipoFactura=B&nroSerie=0&nroFactura=1",
                HttpMethod.GET,
                null,
                new ParameterizedTypeReference<PaginaRespuestaRest<FacturaVenta>>() {})
            .getBody()
            .getContent();
    this.shouldCrearRecibo(5992.5);
    assertTrue(
        "El saldo de la cuenta corriente no es el esperado",
        restTemplate
                .getForObject(apiPrefix + "/cuentas-corriente/clientes/1/saldo", BigDecimal.class)
    ...
    String uri = apiPrefix + "/productos/disponibilidad-stock?idProducto=1,2&cantidad=10,4";
    Assert.assertTrue(
        restTemplate
            .exchange(
                uri, HttpMethod.GET, null, new ParameterizedTypeReference<Map<Long, Producto>>() {})
    ...
    uri = apiPrefix + "/productos/disponibilidad-stock?idProducto=1,2&cantidad=5,4";
    Assert.assertTrue(
        restTemplate
            .exchange(
                uri, HttpMethod.GET, null, new ParameterizedTypeReference<Map<Long, Producto>>() {})
...
//        restTemplate.postForObject(apiPrefix + "/notas/credito/empresa/1/usuario/1/factura/2?idProveedor=1&movimiento=COMPRA&modificarStock=true", notaCreditoProveedor, NotaCredito.class);
...
//        restTemplate.postForObject(apiPrefix + "/notas/debito/empresa/1/usuario/1/recibo/3?idProveedor=1&movimiento=COMPRA", notaDebito, NotaDebito.class);
//        renglonesCuentaCorriente = restTemplate
//                .exchange(apiPrefix + "/cuentas-corriente/1/renglones"
//                        + "?pagina=" + 0 + "&tamanio=" + 50, HttpMethod.GET, null,
//                        new ParameterizedTypeReference<PaginaRespuestaRest<RenglonCuentaCorriente>>() {
...
//        notaDebito.setUsuario(credencial);
//        restTemplate.postForObject(apiPrefix + "/notas/debito/empresa/1/usuario/1/recibo/2?idCliente=1&movimiento=VENTA", notaDebito, NotaDebito.class);
