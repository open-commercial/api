...
class FacturaCompraServiceImplTest {
  ...
  void shouldCalcularIVANetoWhenCompraConFacturaA() {
    ...
    assertEquals(
        0,
        facturaServiceImpl
            .calcularIVANetoRenglon(Movimiento.COMPRA, TipoDeComprobante.FACTURA_A, producto, BigDecimal.ZERO)
  ...
  void shouldCalcularIVANetoWhenCompraConFacturaB() {
    ...
    assertEquals(
        42,
        facturaServiceImpl
            .calcularIVANetoRenglon(Movimiento.COMPRA, TipoDeComprobante.FACTURA_B, producto, BigDecimal.ZERO)
            .doubleValue());
  }
  ...
  void shouldTestBusquedaFacturaCompraCriteria() {
    BusquedaFacturaCompraCriteria criteria =
        BusquedaFacturaCompraCriteria.builder()
            ...
            .fechaFacturaDesde(LocalDateTime.MIN)
            .fechaFacturaHasta(LocalDateTime.MIN)
            .idProveedor(2L)
  ...
  void shouldThrownBusinessServiceExceptionPorBusquedaCompraSinIdSucursal() {
    ...
    assertThrows(BusinessServiceException.class, () -> facturaCompraServiceImpl.getBuilderCompra(criteria));
    verify(messageSource).getMessage(eq("mensaje_busqueda_sin_sucursal"), any(), any());
  }

  @Test
  void shouldCalcularTotalFacturadoCompra() {
    BusquedaFacturaCompraCriteria criteria = BusquedaFacturaCompraCriteria.builder().idSucursal(1L).build();
