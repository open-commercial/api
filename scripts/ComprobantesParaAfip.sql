...
WHERE (factura.tipoComprobante = 'FACTURA_A' OR factura.tipoComprobante = 'FACTURA_B')
	AND (factura.fecha >= '2018-01-01 00:00:00' AND factura.fecha <= '2018-01-31 23:59:59')
...
FROM nota INNER JOIN notacredito on nota.idNota = notacredito.idNota
	INNER JOIN notacreditocliente on notacredito.idNota = notacreditocliente.idNota
	INNER JOIN cliente ON notacreditocliente.id_Cliente = cliente.id_Cliente INNER JOIN condicioniva ON cliente.id_CondicionIVA = condicioniva.id_CondicionIVA
    INNER JOIN localidad ON cliente.id_Localidad = localidad.id_Localidad INNER JOIN provincia ON localidad.id_Provincia = provincia.id_Provincia
WHERE (nota.tipoComprobante = 'NOTA_CREDITO_A' OR nota.tipoComprobante = 'NOTA_CREDITO_B')
	AND (nota.fecha >= '2018-01-01 00:00:00' AND nota.fecha <= '2018-01-31 23:59:59')
...
FROM nota INNER JOIN notadebito on nota.idNota = notadebito.idNota
	INNER JOIN notadebitocliente on notadebito.idNota = notadebitocliente.idNota
	INNER JOIN cliente ON notadebitocliente.id_Cliente = cliente.id_Cliente INNER JOIN condicioniva ON cliente.id_CondicionIVA = condicioniva.id_CondicionIVA
    INNER JOIN localidad ON cliente.id_Localidad = localidad.id_Localidad INNER JOIN provincia ON localidad.id_Provincia = provincia.id_Provincia
